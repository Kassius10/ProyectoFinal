...
function fromPatternImages(pattern) {
    const images = [];
    const array = pattern.toArray();
    for (let i = 0; i < array.length; i++) {
        images.push(PatternItem.fromNative(array[i]));
    }
    return images;
}
function intoPatternImages(pattern) {
...
export class MapView extends MapViewBase {
    ...
    createNativeView() {
        ...
        this._listener = new com.google.android.gms.maps.OnMapReadyCallback({
            onMapReady(map) {
                ...
                org.nativescript.plugins.google_maps.GoogleMaps.registerMapListeners(map, new org.nativescript.plugins.google_maps.GoogleMaps.Callback({
                    onCameraEvent(position, event, isGesture) {
                        if (event === 'start') {
                            ref?.get?.().notify({
                    ...
                    onMapClickEvent(latLng, isLongClick) {
                        if (isLongClick) {
                            ref?.get?.().notify({
                    ...
                    onInfoWindowEvent(marker, event, isLongClick) {
                        let info = {
                    ...
                    onInfoWindowAdapterEvent(marker, event) {
                        const owner = ref.get?.();
                        if (owner) {
                            let info = {
                                eventName: '',
                            ...
                            if (info) {
                                ...
                                if (info.view instanceof View) {
                                    ...
                                    if (info.view.nativeView && !marker._view) {
                                        marker._view = new android.widget.RelativeLayout(owner._context);
                                    }
                                    const parent = info.view.nativeView?.getParent?.();
                                    if (info.view.nativeView && parent !== marker._view) {
                                        if (parent && parent.removeView) {
                                            parent.removeView(info.view.nativeView);
                                        }
                                        const container = marker._view;
                                        container.addView(info.view.nativeView);
                                    }
                                    return marker?._view ?? null;
                                }
                            }
                        }
                        return null;
                    },
                    onIndoorEvent(object, name) {
    ...
    initNativeView() {
        ...
        const ref = new WeakRef(this);
        this.lifeCycleHooks = new android.app.Application.ActivityLifecycleCallbacks({
            onActivityCreated(param0, param1) {
            ...
            onActivityStarted(param0) {
                const owner = ref?.get?.();
                if (!owner?._destroyed && owner?.mapView) {
                    owner.mapView.onStart();
                }
            },
            onActivityResumed(param0) {
                const owner = ref?.get?.();
                if (!owner?._destroyed && owner?.mapView) {
                    owner.mapView.onResume();
                }
            },
            onActivityPaused(param0) {
                const owner = ref?.get?.();
                if (!owner?._destroyed && owner?.mapView) {
                    owner.mapView.onPause();
                }
            },
            onActivityStopped(param0) {
                const owner = ref?.get?.();
                if (!owner?._destroyed && owner?.mapView) {
                    owner.mapView.onStop();
                }
            },
            onActivitySaveInstanceState(param0, param1) {
                if (param0 !== Application.android.startActivity) {
                    return;
                }
                const owner = ref?.get?.();
...
export class Polygon extends OverLayBase {
    ...
    get points() {
        const array = this.native.getPoints().toArray();
        const points = [];
        for (let i = 0; i < array.length; i++) {
            const point = array[i];
            points.push({
    ...
    get holes() {
        const array = this.native.getHoles().toArray();
        const holes = [];
        for (let i = 0; i < array.length; i++) {
            const hole = array[i];
            holes.push({
...
export class Tile {
    ...
    static fromImageSource(source) {
        if (source instanceof ImageSource && source.android) {
            return Tile.fromNative(org.nativescript.plugins.google_maps.GoogleMaps.bitmapToTile(source.android));
        }
        return null;
    }
    get native() {
...
export class TileProvider {
    constructor(callback) {
        if (typeof callback === 'function') {
            ...
            const ref = new WeakRef(this);
            this._native = new com.google.android.gms.maps.model.TileProvider({
                getTile(x, y, zoom) {
                    const owner = ref.get();
                    if (owner) {
...
export class CustomCap extends Cap {
    constructor(nativeCapOrImage, refWidth) {
        super();
...
export class PatternItem {
    static fromNative(nativePattern) {
