...
function isProviderEnabled(provider) {
    try {
        const locationManager = Utils.android.getApplicationContext().getSystemService(android.content.Context.LOCATION_SERVICE);
...
function _requestLocationPermissions(always) {
    return new Promise(function (resolve, reject) {
        if (LocationManager.shouldSkipChecks()) {
            ...
            if (always) {
                ...
                permissions.requestPermissions([android.Manifest.permission.ACCESS_FINE_LOCATION, android.Manifest.permission.ACCESS_COARSE_LOCATION])
                    .then((value) => {
                    resolve(value);
                }).catch((err) => {
...
function _getLocationListener(maxAge, onLocation, onError) {
    return _getTaskSuccessListener((nativeLocation) => {
 ...
 */
function _systemDialogWillShow(always) {
    const permissionType = always ? android.Manifest.permission.ACCESS_BACKGROUND_LOCATION : android.Manifest.permission.ACCESS_FINE_LOCATION;
...
function _permissionIsGiven(always) {
    return always ? permissions.hasPermission(android.Manifest.permission.ACCESS_BACKGROUND_LOCATION) : permissions.hasPermission(android.Manifest.permission.ACCESS_FINE_LOCATION);
}
export function isEnabled(options) {
    return new Promise(function (resolve, reject) {
        if (!_isGooglePlayServicesAvailable() || !permissions.hasPermission(android.Manifest.permission.ACCESS_FINE_LOCATION) && !permissions.hasPermission(android.Manifest.permission.ACCESS_COARSE_LOCATION)) {
            resolve(false);
        }
        else {
...
export class LocationManager {
    static getLastLocation(maximumAge, resolve, reject) {
    ...
    static requestLocationUpdates(locationRequest, locationCallback) {
        _ensureLocationClient();
